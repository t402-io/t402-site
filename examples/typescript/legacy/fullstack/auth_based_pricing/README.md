# SIWE + JWT + Conditional t402 Demo

This project demonstrates a pattern for combining Sign-In with Ethereum (SIWE), JWT-based session management, and dynamic, conditional pricing for API endpoints using t402 payments. It includes a Hono backend server and a separate client script to simulate user interactions.
...
3.  **t402 Programmable Payments**:
    *   API endpoints can require micropayments for access using the [t402 standard](https://t402.dev).
    *   This demo shows manual t402 challenge/verification handling on the server for maximum control, inspired by the t402 SDK's advanced examples.

4.  **Conditional t402 Pricing**:
...
├── README.md         # This file
├── backend.ts        # Hono server with SIWE auth and manual t402 logic
...
*   npm or yarn
*   A testnet wallet with some test ETH (e.g., on Base Sepolia) for `CLIENT_SIM_PRIVATE_KEY` to make t402 payments.
...
*   **Server Terminal**: Shows server startup logs, SIWE nonce issuance, SIWE verification results, JWT pricing decisions, t402 payment verification, and settlement logs.
...
This demonstrates the end-to-end flow of wallet authentication, JWT-based sessions, and dynamic, conditional t402 payments.

## Migrating to Base Mainnet with the CDP Facilitator

To use t402 payments on Base mainnet (instead of Sepolia testnet), follow these steps:
...
- In your `.env` (or environment), set:
  ...
  ```
- **Remove or ignore** any `FACILITATOR_URL` variable. The CDP facilitator is imported from `@coinbase/t402` in code.
...
- **Asset:**
  Make sure your price string (e.g., `$0.01`) maps to a supported asset on Base mainnet (typically USDC). The asset address is handled by the t402 SDK, but your wallet must have the correct token.
- **Funding:**
...
- **Facilitator/Network Mismatch:**
  - Ensure you are using the mainnet facilitator from `@coinbase/t402` and not a testnet URL.
...
*   **t402 SDKs**: Ensure you are using compatible versions of `t402`, `t402-fetch`, and any Hono-specific t402 integrations if you deviate from this demo's manual server-side handling.
