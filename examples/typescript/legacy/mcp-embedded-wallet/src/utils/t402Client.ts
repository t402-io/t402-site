...
import { handle402Error, handleNon402Error, type ErrorHandlingContext } from "./t402ErrorHandler";
import { getCurrentUser, toViemAccount } from "@coinbase/cdp-core";

// Helper types for the MCP tool
export type T402RequestParams = {
  baseURL: string;
  path: string;
...
/**
 * Makes an HTTP request with t402 payment handling capabilities.
 ...
 */
export async function makeT402Request({
  baseURL,
  path,
  method,
  queryParams,
  ...
  const finalCorrelationId =
    correlationId || `t402-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  try {
    ...
    // Make the request (interceptor and t402 library will handle payment flow)
