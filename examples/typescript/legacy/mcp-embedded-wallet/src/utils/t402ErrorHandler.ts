...
import { type t402Response } from "t402/types";
...
function handlePaymentRejection(error: unknown, correlationId: string): void {
  ...
  if (error && typeof error === "object" && "response" in error) {
    const axiosError = error as AxiosError<t402Response>;
    const responseData = axiosError.response?.data as t402Response;
...
export function handleNon402Error(error: unknown, context: ErrorHandlingContext): void {
  ...
  throw new Error(`T402 request failed: ${errorMessage}`);
}
...
function logErrorDetails(error: unknown, context: ErrorHandlingContext): void {
  ...
  console.error("T402 request error details:", JSON.stringify(errorDetails, null, 2));
}
